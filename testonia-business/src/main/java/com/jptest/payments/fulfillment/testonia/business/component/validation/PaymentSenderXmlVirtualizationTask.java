 ...
 */
public class PaymentSenderXmlVirtualizationTask extends XmlVirtualizationTask<Document> {
    ...
    private void init() {
        String virtualizeSchemaFileLocation = configuration
                .getString(BizConfigKeys.PAYMENT_SENDER_VIRTUALIZE_SCHEMA_FILE_LOCATION.getName());
        try {
            populateVirtualizeSchema(virtualizeSchemaFileLocation);
        } catch (IOException e) {
            throw new TestExecutionException("Error reading payment-sender virtualize schema file", e);
        }

    }

    @Override
    public Document pvexecute(Document document) {
