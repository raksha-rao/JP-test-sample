...
/**
 * FulfillPaymentResponse details is used in all the Risk Hold and Reserve API. If the test cases uses PreCreated
 * transaction then there will not be any FulfillPaymentResponse. Need to construct FulfillPaymentResponse using
 * WTRANSACTION_ID_KEY and Store it in FULFILL_PAYMENT_RESPONSE_KEY
 */

public class LegacyFulfillPaymentResponseGenerationTask extends BaseTask<FulfillPaymentResponse> {

    private static final Logger LOGGER = LoggerFactory.getLogger(LegacyFulfillPaymentResponseGenerationTask.class);
    ...
    /**
     * <li>FulfillPaymentResponse details is used in all the Risk Hold and Reserve API. If the test cases uses
     * PreCreated transaction then there will not be any FulfillPaymentResponse.</li>
    ...
    public final FulfillPaymentResponse process(final Context context) {
        ...
        final String sellerSideEncryptedWTransactionID = this.getEncryptedWTransactionID(sellerAccountNumber);

        fulfillPaymentResponse.setFulfillmentHandle(sellerSideEncryptedWTransactionID);
        fulfillPaymentResponse.getTransactionUnitStatus().get(0)
                .setTransactionUnitHandle(sellerSideEncryptedWTransactionID);
        fulfillPaymentResponse.getTransactionUnitStatus().get(0).getHandleDetails()
                .setCreditSideHandle(sellerSideEncryptedWTransactionID);
        fulfillPaymentResponse.getTransactionUnitStatus().get(0).getHandleDetails()
    ...
    /**
     * DebitSide and CreditSide Handles are encrypted wtransaction id. Need to encrypt the precreated transaction id's
     *
     * @param accountNumber
