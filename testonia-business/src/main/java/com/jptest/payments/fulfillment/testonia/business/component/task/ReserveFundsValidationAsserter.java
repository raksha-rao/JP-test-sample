...
/**
 * Valdiatin Reserve Fund Transactions
 */
public class ReserveFundsValidationAsserter extends MassPayBaseValidationAsserter {

	private final static String TYPE_5 = "5";
	private final static String TYPE_SPT = "SPT";
	private final static String TYPE_BAL = "BAL";
	private final static String REASON = "L";
	private final static String SUBTYPE_CODE = "L";
    ...
    public ReserveFundsValidationAsserter(TestCaseInputData testCaseInputData) {
        ...
        LOGGER.info("PAYMENT SIDE ID IS :{} ", debitPaymentSideId);
			...
			Assert.assertEquals(
					this.masspaytxnmapmoneymovementdao.getTransactionMMCount(
							massPayValidationInput.getSender().getAccountNumber(), TYPE_BAL, SUBTYPE_CODE_HAV,
							DEBIT_CODE, debitPaymentSideId.toString()),
			...
			Assert.assertEquals(this.masspaytxnmapmoneymovementdao.getTransactionMMCount(
					massPayValidationInput.getSender().getAccountNumber(), TYPE_5, SUBTYPE_CODE_MLP, REASON,
					CREDIT_CODE, massPayValidationInput.getDecyrptTransactionId(), debitPaymentSideId.toString()),
					RESULT_COUNT, this.getClass().getSimpleName() + ".validate() failed for MoneyMovement table count");
    }
	...
	private static class RetriableMassPayAsserter extends RetriableTask<MassPayValidationInputTask, Integer> {
		...
		protected Integer retriableExecute(MassPayValidationInputTask input) {
			...
			resultCount = wTransactionDao.getType5RowCount(new BigInteger(input.getSender().getAccountNumber()), TYPE_5,
					Long.toString(amt),
					input.getFulfillmentOptions().getTxnAmount().getCurrencyCode(), SUBTYPE_CODE, STATUS_SUCCESS);
			return resultCount;
