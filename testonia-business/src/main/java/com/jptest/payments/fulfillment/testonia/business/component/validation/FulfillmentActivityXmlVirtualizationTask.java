...
/**
 * Performs virtualization on fulfillment activity validation response XML
 */
public class FulfillmentActivityXmlVirtualizationTask extends XmlVirtualizationTask<Document> {
    ...
    private void init() {
        String virtualizeSchemaFileLocation = configuration
                .getString(BizConfigKeys.FULFILLMENT_ACTIVITY_VIRTUALIZE_SCHEMA_FILE_LOCATION.getName());
        try {
            populateVirtualizeSchema(virtualizeSchemaFileLocation);
        } catch (IOException e) {
            throw new TestExecutionException("Error reading fulfillment activity virtualize schema file", e);
        }
    }
