...
/**
 * Represents WTRANSACTION_AUTH table of MONEY database
 */
@Singleton
public class WTransactionAuthDao extends MoneyDao {

    private static final String GET_WTRANSACTION_AUTH_QUERY = "SELECT * FROM WTRANSACTION_AUTH where transaction_id in ({commaSeparatedIds}) order by time_created,transaction_id";
    ...
    /**
     * Queries WTRANSACTION_AUTH table for input transaction
     * @param transactions
     * @return
     */
    public List<WTransactionAuthDTO> getWTransactionAuthDetails(List<WTransactionDTO> transactions) {
        String query = GET_WTRANSACTION_AUTH_QUERY.replace(COMMA_SEPARATED_IDS_REPLACEMENT_TOKEN,
