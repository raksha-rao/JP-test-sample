...
/**
 * Represents WWAX_TRANSACTION_P2 table of TXN database
 */
@Singleton
public class WWaxTransactionP2Dao extends TxnDao {

    private static final String GET_WWAX_TRANSACTION_P2_DETAILS_QUERY = "SELECT * FROM WWAX_TRANSACTION_P2 where trans_data_map_id in "
            + "(select MAP_ID from WTRANS_DATA_MAP_P2 where transactionlike_id in ({commaSeparatedIds}) and transactionlike_type = 'T' and BITAND(FLAGS, {wwaxTransaction}) > 0 ) "
    ...
    /**
     * Queries WWAX_TRANSACTION_P2 table for input transaction
     * @param transactions
     * @return
     */
    public List<WwaxTransactionDTO> getWwaxTransactionDetails(List<WTransactionDTO> transactions) {
        String query = GET_WWAX_TRANSACTION_P2_DETAILS_QUERY.replace(COMMA_SEPARATED_IDS_REPLACEMENT_TOKEN,
