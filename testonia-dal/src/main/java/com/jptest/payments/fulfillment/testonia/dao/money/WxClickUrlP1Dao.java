...
/**
 * Represents WXCLICK_URL table of MONEY database
 */
@Singleton
public class WxClickUrlP1Dao extends MoneyDao {

    private static final String GET_WXCLICK_URL_DETAILS_QUERY = "SELECT * FROM WXCLICK_URL where trans_data_map_id in "
            + "(select MAP_ID from WTRANS_DATA_MAP where transactionlike_id in ({commaSeparatedIds}) and transactionlike_type = 'T' and BITAND(FLAGS, 2) > 0) "
    ...
    public List<WxClickUrlDTO> getWxClickUrlDetails(List<WTransactionDTO> transactions) {
        String query = GET_WXCLICK_URL_DETAILS_QUERY.replace(COMMA_SEPARATED_IDS_REPLACEMENT_TOKEN,
