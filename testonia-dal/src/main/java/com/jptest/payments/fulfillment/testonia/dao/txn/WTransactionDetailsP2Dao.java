...
/**
 * Represents WTRANSACTION_DETAILS_P2 table of TXN database
 */
@Singleton
public class WTransactionDetailsP2Dao extends TxnDao {

    private static final String GET_TRANSACTION_DETAILS_QUERY = "SELECT * FROM WTRANSACTION_DETAILS_P2 where TRANS_DATA_MAP_ID in "
            + "(select MAP_ID from WTRANS_DATA_MAP_P2 where transactionlike_id in ({commaSeparatedIds}) and transactionlike_type = 'T' and BITAND(FLAGS, {wTransactionDetailFlag}) > 0) "
            + "order by time_created,id";
    private static final String TRANSACTION_DETAIL_FLAG_REPLACEMENT_TOKEN = "{wTransactionDetailFlag}";
    private static final String TRANSACTION_DETAIL_FLAG_VALUE = "512";
    ...
    public List<WTransactionDetailsDTO> getWTransactionDetails(List<WTransactionDTO> transactions) {
        String query = GET_TRANSACTION_DETAILS_QUERY.replace(COMMA_SEPARATED_IDS_REPLACEMENT_TOKEN,
                ...
                .replace(TRANSACTION_DETAIL_FLAG_REPLACEMENT_TOKEN, TRANSACTION_DETAIL_FLAG_VALUE);
