...
/**
 * Represents WSUBBALANCE_TRANSACTION table of TXN database
 */
@Singleton
public class WTransactionAdminP2Dao extends TxnDao {

    private static final String GET_TRANSACTION_ADMIN_P2_DETAILS_QUERY = "SELECT * FROM WTRANSACTION_ADMIN_P2 where transaction_id in ({commaSeparatedIds}) order by id";
    ...
    /**
     * Queries WTRANSACTION_ADMIN_P2 table for input transaction
     * @param transactions
     * @return
     */
    public List<WTransactionAdminDTO> getWTransactionAdminDetails(List<WTransactionDTO> transactions) {
        String query = GET_TRANSACTION_ADMIN_P2_DETAILS_QUERY.replace(COMMA_SEPARATED_IDS_REPLACEMENT_TOKEN,
