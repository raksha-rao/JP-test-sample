...
/*
 * Represents WTRANS_DATA_MAP in MONEY DB
 */
public class WTransDataMapP1Dao extends MoneyDao {

    private static final String GET_WTRANS_DATA_MAP = "select * from WTRANS_DATA_MAP where transactionlike_id in ({commaSeparatedIds}) and transactionlike_type = 'T' and BITAND(FLAGS, 32) > 0 order by map_id";
    ...
    public List<WTransDataMapDTO> getTransDataMapDetails(List<WTransactionDTO> transactions) {
        String query = GET_WTRANS_DATA_MAP.replace(COMMA_SEPARATED_IDS_REPLACEMENT_TOKEN,
