...
/**
 * Represents WAUCTION_ITEM_ADDL_ATTR_P2 table of TXN database
 */
@Singleton
public class WAuctionItemAddlAttrP2Dao extends TxnDao {

    private static final String GET_WAUCTION_ITEM_ADDL_ATTR_P2_DETAILS_QUERY = "SELECT * FROM WAUCTION_ITEM_ADDL_ATTR_P2 where auction_item_id in ({auctionItemIds}) order by auction_item_id";
    ...
    /**
     * Queries WAUCTION_ITEM_ADDL_ATTR_P2 table for input transaction
     * @param commaSeparatedAuctionItemIds
     * @return
     */
    public List<WAuctionItemAddlAttrDTO> getWAuctionItemAdditionalAttributesDetails(
            String commaSeparatedAuctionItemIds) {
        String query = GET_WAUCTION_ITEM_ADDL_ATTR_P2_DETAILS_QUERY.replace(AUCTION_ITEM_ID_REPLACEMENT_TOKEN,
