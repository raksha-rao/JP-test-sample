...
/**
 * Represents WTRANSACTION_URL_P2 table of TXN database
 */
@Singleton
public class WTransactionUrlP2Dao extends TxnDao {

    private static final String GET_TRANSACTION_URL_P2_DETAILS_QUERY = "SELECT * FROM WTRANSACTION_URL_P2 where trans_data_map_id in "
            + "(select MAP_ID from WTRANS_DATA_MAP_P2 where transactionlike_id in ({commaSeparatedIds}) and transactionlike_type = 'T' and BITAND(FLAGS, {wTransactionUrlFlag}) > 0) "
    ...
    /**
     * Queries WTRANSACTION_URL_P2 table for input transaction
     * @param transactions
     */
    public List<WTransactionUrlDTO> getWTransactionUrlDetails(List<WTransactionDTO> transactions) {
        String query = GET_TRANSACTION_URL_P2_DETAILS_QUERY.replace(COMMA_SEPARATED_IDS_REPLACEMENT_TOKEN,
