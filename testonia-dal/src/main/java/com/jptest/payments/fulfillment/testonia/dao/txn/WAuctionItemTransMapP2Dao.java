...
/**
 * Represents WAUCTION_ITEM_TRANS_MAP_P2 table of TXN database
 */
@Singleton
public class WAuctionItemTransMapP2Dao extends TxnDao {

    private static final String GET_WAUCTION_ITEM_TRANS_MAP_P2_DETAILS_QUERY = "SELECT * FROM WAUCTION_ITEM_TRANS_MAP_P2 where payer_transaction_id in ({commaSeparatedIds}) order by payer_transaction_id";
    ...
    /**
     * Queries WAUCTION_ITEM_TRANS_MAP_P2 table for input transaction
     * @param transactions
     * @return
     */
    public List<WAuctionItemTransMapDTO> getWAuctionItemTransMapDetails(List<WTransactionDTO> transactions) {
        String query = GET_WAUCTION_ITEM_TRANS_MAP_P2_DETAILS_QUERY.replace(COMMA_SEPARATED_IDS_REPLACEMENT_TOKEN,
